...
#  smurt.pl - Version 1.0.0  07 Nov 20
...
#    chmod 700 /usr/local/bin/smurt.pl
...
my $strServer = 'lab.server.com';  #server name
...
sub processMail
{
  ...
  if ($strMTA ne '')
  {
    ...
    print OUTLOG "$strLdate $addrClient->$addrServer $idMail " . (-s $fileMail) . " \"$strMTA\" \"$strFrom\" \"$strSubject\"\n";  #write mail fields to log
